...
def menu(exits):
    ...
    while True:
        ...
        print_menu(exits)
        # Read player's input
        user_command = input("please type the name of the exit  ")
        # Normalise the input
        user_command = normalise_input(user_command)

        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if is_valid_exit(exits, user_command):
            return user_command
